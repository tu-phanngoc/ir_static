...
static bool     m_busy;
static uint32_t m_ir_pin = 0xFFFFFFFF;
static void (*tx_done_cb)(void);

static bool is_ir_lib_init = false;



void ir_lib_tx_done_callback()
{
	if(tx_done_cb != NULL) {
		tx_done_cb();
	}
...
{
	if(is_ir_lib_init == false)
	{
		return 1;
	}
	is_ir_lib_init = false;
	NRF_GPIOTE->CONFIG[0] &= ~(GPIOTE_CONFIG_MODE_Task         << GPIOTE_CONFIG_MODE_Pos |
                            GPIOTE_CONFIG_OUTINIT_Low       << GPIOTE_CONFIG_OUTINIT_Pos |
                            GPIOTE_CONFIG_POLARITY_Toggle   << GPIOTE_CONFIG_POLARITY_Pos |
                            m_ir_pin                          << GPIOTE_CONFIG_PSEL_Pos);
	sd_nvic_DisableIRQ(IR_CARRIER_COUNTER_IRQn);
...
uint32_t ir_lib_init(uint32_t ir_pin, void* cb)
{
    ...
    IR_TIMER_CARRIER->PRESCALER     = 0; //4
...
{
    if(IR_CARRIER_COUNTER->EVENTS_COMPARE[m_next_cc_for_update])
    {
        ...
        else if(m_bits_remaining == 0)
        {
            IR_TIMER_CARRIER->TASKS_STOP = 1;
						IR_CARRIER_COUNTER->TASKS_STOP = 1;
//						m_busy = false;
						ir_lib_tx_done_callback();
        }

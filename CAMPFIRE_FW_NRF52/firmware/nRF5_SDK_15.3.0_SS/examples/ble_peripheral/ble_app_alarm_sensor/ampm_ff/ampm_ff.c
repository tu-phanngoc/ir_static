...
#include "ampm_ff.h"
#include "nrf_log.h"
#define AMPM_FF_DBG(...)		NRF_LOG_INFO(__VA_ARGS__)
...
AMPM_FRESULT  ampm_f_open(AMPM_FIL *fil,char  *fileName,uint8_t mode)
{
	...
	{
		if(ampm_f_searchfile(fil,fileName) == FR_OK)
		{
	...
	{
		if(ampm_f_find_free_sector(&i) == FR_OK)
		{
			if(mode & (AMPM_FA_CREATE_ALWAYS | AMPM_FA_CREATE_NEW))
			{
				AMPM_FF_DBG("ampm_f_open:create a new file->");
...
AMPM_FRESULT  ampm_f_isfile(AMPM_FIL *fil)
{
	uint32_t i;
	for(i = 0; i < DISK_SECTOR_MAX;i++)
	{
...
AMPM_FRESULT  ampm_f_searchfile(AMPM_FIL *fil,char *fileName)
{
	uint32_t i;
	for(i = 0; i < DISK_SECTOR_MAX;i++)
	{
...
AMPM_FRESULT ampm_f_readln (
	AMPM_FIL *fp, 		/* Pointer to the file object */
	char* buff,		/* Pointer to data buffer */
	uint32_t btr,		/* Number of bytes to read */
	uint32_t* br		/* Pointer to number of bytes read */
)
{
	...
	for (i = 0; i < btr; i++)
	{
		res = ampm_f_read(fp, rbuff, 1, &n);
		if ((res != FR_OK) || (n != 1))
			return FR_ERR;
		if (((*(rbuff-1)) == 13) && ((*rbuff) == 10))
		{
			//f_read(fp, rbuff, 1, &n);
			*(rbuff-1) = '\0';
			(*br)--;			   	

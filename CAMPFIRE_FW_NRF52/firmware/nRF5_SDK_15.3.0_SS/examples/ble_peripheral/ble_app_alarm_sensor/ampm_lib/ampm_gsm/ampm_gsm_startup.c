...
uint32_t Ampm_GSM_GetSimCIMI(uint16_t cnt,uint8_t c);
uint32_t Ampm_GSM_GetSimCPOS(uint16_t cnt,uint8_t c);

uint8_t gsmSimCIMIBuf[32];
uint8_t flagGotSimCIMI = 0;
uint8_t modemCIMIPass = 0;
...
uint8_t gsmSimCPOSBuf[32];
uint8_t flagGotSimCPOS = 0;
uint8_t modemCPOSPass = 0;

uint8_t gsmIMEIBuf[18];
uint8_t flagGotIMEI = 0;
uint8_t newIMEI_IsUpdate = 0;
uint8_t modemIMEIPass = 0;
uint8_t modemJustPowerOff = 0;
...
//								}
								tryCnt = 3;
								AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:TRY TO TURN ON TIMES = %d \r\n",tryCnt);
								...
								AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:AMPM_GSM_STARTUP_TURN_ON \r\n");
        break;
        case AMPM_GSM_STARTUP_TURN_ON:
                if (CheckTimeout(&tAmpm_GsmStartupTaskDelay) == SYSTICK_TIMEOUT)
                {
									...
									AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:AMPM_GSM_STARTUP_TURN_ON1 \r\n");
                }
        break;
        case AMPM_GSM_STARTUP_TURN_ON1:
                if (CheckTimeout(&tAmpm_GsmStartupTaskDelay) == SYSTICK_TIMEOUT)
                {
									...
									AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:AMPM_CMD_OK \r\n");
                }
        break;
        case AMPM_GSM_STARTUP_GET_CMD_PHASE:
                if (CheckTimeout(&tAmpm_GsmStartupTaskDelay) == SYSTICK_TIMEOUT)
                {
                        if(ampm_GSM_CmdPhase == AMPM_CMD_OK)
                        {
                                ...
                                {
                                  ampm_GSM_StartupPhase = AMPM_GSM_STARTUP_INIT_PHASE;
																	AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:AMPM_GSM_STARTUP_INIT_PHASE \r\n");
                                ...
                                switch(at_cmd.cmdType)
                                {
                                        ...
                                        if(at_cmd.timeout == 0) at_cmd.timeout = 1000;
																				switch(callback)
																				{
																					...
																					break;
																					case 5:
																						at_cmd.callback = GsmGetUTCTime;
																					break;
																					case 6:
																						at_cmd.callback = Ampm_GSM_GetSimCIMI;
																					break;
																					case 7:
																						at_cmd.callback = Ampm_GSM_GetSimCPOS;
																					break;
																					default:
																						at_cmd.callback = NULL;
																					break;
                                        ...
                                        case 'S':
																					...
																					AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:AMPM_GSM_STARTUP_IDLE_PHASE \r\n");
																					break;
                                        default:
																					ampm_GSM_StartupPhase = AMPM_GSM_STARTUP_INIT_PHASE;
                                        break;
...
uint32_t Ampm_GSM_GetIMEI(uint16_t cnt,uint8_t c)
{
	...
	{
		AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:Got IMEI:%s \r\n",gsmIMEIBuf);
		flagGotIMEI = 1;
		modemIMEIPass = 1;
		return 0;
	}
...
uint32_t Ampm_GSM_GetSimCID(uint16_t cnt,uint8_t c)
{
	...
	{
		AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:Got CID:%s \r\n",gsmSimCIDBuf);
		flagGotSimCID = 1;
		modemCIDPass = 1;
		return 0;
	}
...
uint32_t Ampm_GSM_GetSimCIMI(uint16_t cnt,uint8_t c)
{
	...
	{
		AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:Got CIMI:%s \r\n",gsmSimCIMIBuf);
		flagGotSimCIMI = 1;
		modemCIMIPass = 1;
		return 0;
	}
...
uint32_t Ampm_GSM_GetSimCPOS(uint16_t cnt,uint8_t c)
{
	...
	{
		AMPM_GSM_LIB_DBG(" AMPM_GSM_INIT:Got CPOS:%s \r\n",gsmSimCPOSBuf);
		flagGotSimCPOS = 1;
		modemCPOSPass = 1;
		return 0;
	}

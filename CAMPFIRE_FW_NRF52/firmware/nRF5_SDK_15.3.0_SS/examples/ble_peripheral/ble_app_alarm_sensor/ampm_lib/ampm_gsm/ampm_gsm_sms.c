...
void Ampm_SmsAddMsg(struct SMS_LIST_TYPE *msg);
uint8_t Ampm_GsmSmsRecvCallback_CCLK(uint8_t *buf);
...
uint8_t Ampm_GsmSmsRecv3(AMPM_CMD_PHASE_TYPE phase);
...
uint8_t Ampm_GsmSmsSendCallback_CheckAT_OK(AMPM_CMD_PHASE_TYPE phase);
uint8_t Ampm_GsmSmsRecvCallback_CheckAT_OK(AMPM_CMD_PHASE_TYPE phase);
uint8_t smsTryCnt = 0;
extern const AMPM_GSM_AT_CMD_PACKET_TYPE smsRecvCmd_Read_AT_CCLK;
extern const AMPM_GSM_AT_CMD_PACKET_TYPE sendCmd_AT_CGSN;

const AMPM_GSM_AT_CMD_PACKET_TYPE smsSevdCmd_Set_AT_CCLK = {NULL,Ampm_GsmSmsRecvCallback_CCLK,NULL,NULL,"OK","ERROR"};//delay = 500,timeout = 3000,retry = 1
...
const AMPM_CMD_PROCESS_TYPE smsRecvCmdProcess_03	= {
		(void *)&smsRecvCmdProcess_1,(void *)&ampmAtCmd_AT_CSCS_GSM,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,3000,100};
		
const AMPM_CMD_PROCESS_TYPE smsRecvCmdProcess_02	= {
		(void *)&smsRecvCmdProcess_03,(void *)&sendCmd_AT_CGSN,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,3000,100};
	
const AMPM_CMD_PROCESS_TYPE smsRecvCmdProcess_01	= {
		(void *)&smsRecvCmdProcess_02,(void *)&smsRecvCmd_Read_AT_CCLK,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,3000,100};

const AMPM_CMD_PROCESS_TYPE smsRecvCmdProcess_001	= {
		(void *)&smsRecvCmdProcess_02,(void *)&smsSevdCmd_Set_AT_CCLK,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,3000,100};

const AMPM_CMD_PROCESS_TYPE smsRecvCmdProcess_0	= {
		(void *)&smsRecvCmdProcess_01,(void *)&ampmAtCmd_AT_CSQ,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,Ampm_GsmSmsRecv3,1,1000,100};

const AMPM_CMD_PROCESS_TYPE smsRecvCmdProcess_00	= {
		(void *)&smsRecvCmdProcess_0,(void *)&ampmAtCmd_AT_CEND,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,1000,100};

const AMPM_CMD_PROCESS_TYPE smsRecvCmdProcess_Start1	= {
		(void *)&smsRecvCmdProcess_00,(void *)&ampmAtCmd_GotoCmdMode,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,1000,100};

const AMPM_CMD_PROCESS_TYPE smsRecvCmdProcess_Start	= {
		(void *)&smsRecvCmdProcess_00,(void *)&ampmAtCmd_AT,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,Ampm_GsmSmsRecvCallback_CheckAT_OK,1,1000,100};
...
const AMPM_GSM_AT_CMD_PACKET_TYPE smsSendMessage = {NULL,Ampm_GsmSmsSendMessageCallback,NULL,NULL,"+CMGS:","ERROR"};//delay = 500,timeout = 3000,retry = 1
...
const AMPM_CMD_PROCESS_TYPE smsSendCmdProcess_5	= {
		(void *)&smsSendCmdProcess_end,(void *)&smsSendMessage,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,Ampm_GsmSmsSent,1,30000,100};
...
const AMPM_CMD_PROCESS_TYPE smsSendCmdProcess_CSCS	= {
		(void *)&smsSendCmdProcess_3,(void *)&ampmAtCmd_AT_CSCS_GSM,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,3000,100};
		
const AMPM_CMD_PROCESS_TYPE smsSendCmdProcess_2	= {
		(void *)&smsSendCmdProcess_CSCS,(void *)&ampmAtCmd_AT_CMGF_0,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,Ampm_GsmSmsSendCallback_CSCA,1,1000,100};
...
const AMPM_CMD_PROCESS_TYPE smsSendCmdProcess_0_0	= {
		(void *)&smsSendCmdProcess_0,(void *)&ampmAtCmd_GotoCmdMode,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,1000,100};

const AMPM_CMD_PROCESS_TYPE smsSendCmdProcess_Start	= {
		(void *)&smsSendCmdProcess_0,(void *)&ampmAtCmd_AT,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,1,1000,100};
...
uint8_t Ampm_GsmSmsRecvCallback_CheckAT_OK(AMPM_CMD_PHASE_TYPE phase)
{
	...
	{
		Ampm_SendAtCmdNow(&smsRecvCmdProcess_00);
	}
	else
	{
		Ampm_SendAtCmdNow(&smsRecvCmdProcess_Start1);
	}
	return 1;
}
		...
		else
		{
			if(getServiceCenterNumber && serviceCenterNumber[0] != NULL)
			{
				if(smsMsg->mode == SMS_PDU16_MODE)
					pdu_len = pdu16_encode((const char *)serviceCenterNumber,(const char *)smsMsg->phone,( const unsigned short *)smsMsg->msg,smsMsg->len,smsDataBuf,sizeof(smsDataBuf));
				else
			...
			{
				Ampm_SendAtCmdNow(&smsSendCmdProcess_Start);
			}
		}
...
uint8_t Ampm_GsmSmsRecvCallback_CCLK(uint8_t *buf)
{
	sysGsmTime.year = sysTime.year;
	sysGsmTime.month = sysTime.month;
	sysGsmTime.mday = sysTime.mday;
	sysGsmTime.hour = sysTime.hour;
	sysGsmTime.min = sysTime.min;
	sysGsmTime.sec = sysTime.sec;
	return (uint8_t )ampm_sprintf((char *)buf,"AT+CCLK=\"%02d/%02d/%02d,%02d:%02d:%02d+28\"\r",sysGsmTime.year,sysGsmTime.month,sysGsmTime.mday,sysGsmTime.hour,sysGsmTime.min,sysGsmTime.sec);
}
...
uint8_t Ampm_GsmSmsRecvCallback_CMGD(uint8_t *buf)
{
	...
	//return (uint8_t )ampm_sprintf((char *)buf,"AT+CMGD=-1\r");
}
...
uint8_t Ampm_GsmSmsRecv0(AMPM_CMD_PHASE_TYPE phase)
{
	...
	if(RINGBUF_GetFill(&smsUnreadRingBuff) & 0x01 == 0)
	{
		while(RINGBUF_Get(&smsUnreadRingBuff,&c) == 0);
		Ampm_SendAtCmdNow(&smsRecvCmdProcess_3);
		errorTimes++;
	}
	else if(RINGBUF_GetFill(&smsUnreadRingBuff) >=2)
	{
...
uint8_t Ampm_GsmSmsRecv1(AMPM_CMD_PHASE_TYPE phase)
{
	...
	{
		if(RINGBUF_GetFill(&smsUnreadRingBuff))
		{
			ampm_NewSmsFlag = 1;
			while(RINGBUF_Get(&smsUnreadRingBuff,&c) == 0);
		}
...
uint8_t Ampm_GsmSmsRecv3(AMPM_CMD_PHASE_TYPE phase)
{
	...
	{
		gsmGetTimeFlag = 0;
		Ampm_SendAtCmdNow(&smsRecvCmdProcess_001);
	}
	else
	{
		Ampm_SendAtCmdNow(&smsRecvCmdProcess_01);
	}
	return 1;
}
...
{
		return Ampm_SendAtCmd(&smsRecvCmdProcess_Start);
}
...
uint32_t Ampm_SMS_IsInProcess(struct SMS_LIST_TYPE *my_msg)
{
	static struct SMS_LIST_TYPE *msg = NULL;
...
uint16_t Ampm_Sms_SendMsg(struct SMS_LIST_TYPE *SMS,uint8_t *phone,uint8_t *msg,uint8_t len,SMS_MODE_TYPE mode,uint16_t timeout,uint8_t tryNum)
{
		SMS->msg = msg;
		SMS->len = len;
		SMS->phone = phone;
		...
		SMS->mode = mode;
		AMPM_GSM_LIB_DBG("\r\nsend sms to:%s,len=%d",SMS->phone,SMS->len);
		if(SMS->mode == SMS_TEXT_MODE)
			AMPM_GSM_LIB_DBG(",%s",SMS->msg);
		Ampm_SmsAddMsg(SMS);
		return 0;
}
